...
- (id) initWithAnnotation:(id <MKAnnotation>)annotation reuseIdentifier:(NSString *)reuseIdentifier {
	if (self = [super initWithAnnotation:annotation reuseIdentifier:reuseIdentifier]) {
        ...
        [self.accessory setTitle:@"Como llegar" forState:UIControlStateNormal];
       
        [self.accessory sizeToFit];
        
        
        [self.accessory setEnabled:YES];;
...
- (void)prepareAccessoryFrame {
	self.accessory.frame = CGRectMake(self.bounds.size.width - self.accessory.frame.size.width - 50,
									  (self.contentHeight  - self.accessory.frame.size.height) / 2,
...
- (void) calloutAccessoryTapped {                       //FUNCION DE ACCION BOTON 1
